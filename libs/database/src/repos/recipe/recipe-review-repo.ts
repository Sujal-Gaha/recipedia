...
export class PrismaRecipeReviewRepo extends RecipeReviewRepo {
  ...
  override async update({ id, data: { comment, rating } }: TUpdateRecipeReviewRepoInput): Promise<TRecipeReview> {
    return await db.recipeReview.update({
      where: {
        id,
      },
      data: {
